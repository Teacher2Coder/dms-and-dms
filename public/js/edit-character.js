...
const handleEditCharacter = async (event) => {
  ...
  const pointCount =
    parseInt(strengthInput) +
    parseInt(dexterityInput) +
    parseInt(constitutionInput) +
    parseInt(intelligenceInput) +
    parseInt(wisdomInput) +
    parseInt(charismaInput);
  ...
  if (
    ...
    alert(
      `You still have ${
        75 - pointCount
      } points to distribute for the character's attributes`
    );
    ...
    alert(
      `Take away ${
        pointCount - 75
      } points from the character's attribute scores`
    );
