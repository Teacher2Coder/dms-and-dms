...
router.get('/', async (req, res) => {
    try {
        ...
        res.render('rules', { rulesDataPlain, loggedIn: req.session.loggedIn, user: req.session.user })
    } catch (err) {
        console.error(err);
...
router.get('/:id', async (req, res) => {
    try {
        ...
        res.render('single-rule', { rule, loggedIn: req.session.loggedIn, user: req.session.user })
    } catch (err) {
        console.error(err);
        res.status(500).json(err);
    }
